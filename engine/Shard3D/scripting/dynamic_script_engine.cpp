...
namespace Shard3D {
	...
	struct ScriptEngineData {
		...
		hashMap<std::string, sPtr<DynamicScriptClass>> actorClasses;
		hashMap<UUID, sPtr<DynamicScriptInstance>> actorInstances;

		DynamicScriptClass hudClass;
		hashMap<std::string, sPtr<DynamicScriptClass>> hudClasses;
	...
	void DynamicScriptEngine::loadAssemblyClasses(ScriptEngineData* data) {
		...
		for (int32_t i = 0; i < numTypes; i++) {
			...
			sPtr<DynamicScriptClass> ptr = make_sPtr<DynamicScriptClass>(nameSpace, name, (int)data->lang);
			if (isActor) {
	...
	hashMap<std::string, sPtr<DynamicScriptClass>> DynamicScriptEngine::getActorClasses(ScriptLanguage lang) {
		return !lang ? scriptEngineData->actorClasses : vbScriptEngineData->actorClasses;
	}

	void DynamicScriptEngine::runtimeStart(ECS::Level* level) {
	...
	ECS::Level* DynamicScriptEngine::getContext() {
	...
	void DynamicScriptEngine::_a::beginEvent(ECS::Actor __a) {
		...
		if (doesClassExist("Shard3D.Scripts." + scr.name, scr.lang)) {
			...
			sPtr<DynamicScriptInstance> instance = make_sPtr<DynamicScriptInstance>(data->actorClasses["Shard3D.Scripts." + scr.name], scr.lang, __a);
	...
	void DynamicScriptEngine::_a::spawnEvent(ECS::Actor actor) {
		{ // C#
	...
	void DynamicScriptEngine::_a::killEvent(ECS::Actor actor) {
		{ // C#
			const auto guid = actor.getUUID();
			if (scriptEngineData->actorInstances.find(guid) != scriptEngineData->actorInstances.end()) {
...
namespace Shard3D {
	...
	DynamicScriptInstance::DynamicScriptInstance(sPtr<DynamicScriptClass> s_class, ScriptLanguage lang, ECS::Actor actor) : scriptClass(s_class) {
	...
	DynamicScriptInstance::ScriptEvents::ScriptEvents(sPtr<DynamicScriptClass> ptr, MonoObject* i) : s_c(ptr), _i(i) {
	...
	HUDScriptInstance::HUDScriptInstance(sPtr<DynamicScriptClass> s_class, int lang, HUDElement* element) : scriptClass(s_class) {
		instance = s_class->inst();
	...
	HUDScriptInstance::ScriptEvents::ScriptEvents(sPtr<DynamicScriptClass> ptr, MonoObject* i) : s_c(ptr), _i(i) {
