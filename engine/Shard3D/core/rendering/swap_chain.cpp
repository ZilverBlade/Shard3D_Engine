...
EngineSwapChain::EngineSwapChain(EngineDevice& deviceRef, VkExtent2D extent, sPtr<EngineSwapChain> previous)
    : device{ deviceRef }, windowExtent{ extent } {
    init();

    //clean up old swap chain cuz it's no longer needed
...
VkFormat EngineSwapChain::findDepthFormat() {
    ...
    else return VK_FORMAT_D24_UNORM_S8_UINT;
}
}
