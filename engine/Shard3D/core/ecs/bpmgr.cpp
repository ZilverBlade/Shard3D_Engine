...
#include "level.h" // for the killMesh function
#include <fstream>
...
namespace Shard3D {
	namespace ECS {
		...
		void BlueprintManager::removeAllComponents(Actor actor) {
			//don't remove 3 crutial components like transform, tag and UUID since they must be controllable on any actor
		...
		void BlueprintManager::convert(const std::string& destinationPath, Blueprint blueprint, bool encryptLevel) {
			...
			out << YAML::Key << "BlueprintID" << YAML::Value << blueprint.getUUID();
