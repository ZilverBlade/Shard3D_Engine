...
namespace Shard3D {
	...
	void MaterialSystem::saveMaterial(Material materialData, const std::string& saveLoc) {
		...
		out << YAML::BeginMap;
			...
			if (materialData.surfaceMaterial.surfaceMat == MaskedMaterial || materialData.surfaceMaterial.surfaceMat == MaskedTranslucentMaterial)
	...
	MaterialSystem::Material MaterialSystem::loadMaterial(const std::string& loadLoc, bool ignoreWarns) {
		...
		if (data["Material"]) {
			for (auto container : data["Container"]) {
