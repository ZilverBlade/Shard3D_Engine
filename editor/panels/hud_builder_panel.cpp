...
#include <fstream>
namespace Shard3D {
    std::vector<sPtr<HUDElement>> killQueue_Element;
    ...
    void HUDBuilderPanel::setContext(sPtr<HUDContainer> gctnr) {
        hudLayerInfo = gctnr;
    }
        ...
        for (int layer = 0; layer < hudLayerInfo->getList().size(); layer++) {
            if (ImGui::TreeNode(std::string("Layer " + std::to_string(layer)).c_str())) {
                ...
                if (ImGui::Button("Add Element")) {
                    sPtr<HUDElement> hudElement = make_sPtr<HUDElement>();
                    hudElement->guid = UUID();
    ...
    void HUDBuilderPanel::renderGUIElementProperties(sPtr<HUDElement> element, int layer) {
