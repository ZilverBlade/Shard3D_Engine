...
namespace Shard3D.Scripts
{
	public class CarDriver : Actor
	{
		...
		protected void TickEvent(float dt)
		{
			...
			if (InternalCalls.IsKeyDown(KeyInput.KEY_D))
			{
				rotationWFR.z += 0.4f * dt;
				rotationWFR.z = Math.Min(rotationWFR.z, 0.5f);
			}
			if (InternalCalls.IsKeyDown(KeyInput.KEY_A))
			{
				rotationWFR.z -= 0.4f * dt;
				rotationWFR.z = Math.Max(rotationWFR.z, -0.5f);
			}
			if (InternalCalls.IsKeyDown(KeyInput.KEY_S))
				moveVector -= dt;
			if (InternalCalls.IsKeyDown(KeyInput.KEY_W))
			{
				exponent += dt * 0.05f; 
