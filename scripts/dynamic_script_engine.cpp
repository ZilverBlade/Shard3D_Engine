...
namespace Shard3D {
	...
	void DynamicScriptEngine::initMono(const char* lang, ScriptEngineData* scriptEngine) {
		...
		// C#
		if (lang == "CS")
			scriptEngine->appDomain = mono_domain_create_appdomain((char*)"Shard3D_CS_Runtime", nullptr);
		else if (lang == "VB")
			scriptEngine->appDomain = mono_domain_create_appdomain((char*)"Shard3D_VB_Runtime", nullptr);

		mono_domain_set(scriptEngineData->appDomain, true);

		if (lang == "CS")
			scriptEngine->coreAssembly = loadAssembly(ENGINE_CS_SCRIPT_RUNTIME_DLL);
		else if (lang == "VB")
			scriptEngine->coreAssembly = loadAssembly(ENGINE_VB_SCRIPT_RUNTIME_DLL);

		printAssemblyTypes(scriptEngine->coreAssembly, lang != "CS");

		MonoImage* assemblyImage = mono_assembly_get_image(scriptEngine->coreAssembly);
		MonoClass* monoClass = mono_class_from_name(assemblyImage, 
			std::string("Shard3D." + std::string(lang) + "Scripts").c_str(), // default namespace 
			"Main");
