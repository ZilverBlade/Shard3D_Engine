#include "../../s3dtpch.h"
#include "editor_movement_controller.hpp"
#include <limits>
...
namespace Shard3D {
	namespace controller {
		void EditorMovementController::tryPoll(GLFWwindow* _window, float _dt, wb3d::Actor& _actor) {
			window = _window;
			actor = _actor;
			dt = _dt;
			SHARD3D_EVENT_BIND_HANDLER(EditorMovementController::eventEvent);
		...
		void EditorMovementController::eventEvent(Event& e) {
			...
			SHARD3D_EVENT_DISPATCH(MouseScrollEvent, controller::EditorMovementController::mouseScrollEvent);
		}
		bool EditorMovementController::keyDownEvent(KeyDownEvent& e) {
			float yaw = actor.getComponent<Components::TransformComponent>().getRotation().z;
			float pitch = actor.getComponent<Components::TransformComponent>().getRotation().x;
			const glm::vec3 forwardDir{ sin(yaw), cos(yaw), -sin(pitch) };
			const glm::vec3 rightDir{ forwardDir.y, -forwardDir.x, 0.f, };
		...
		bool EditorMovementController::mouseButtonDownEvent(MouseButtonDownEvent& e) {
		...
		bool EditorMovementController::mouseScrollEvent(MouseScrollEvent& e) {
